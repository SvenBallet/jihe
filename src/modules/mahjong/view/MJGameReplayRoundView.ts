module FL {
     ...
     * 麻将切换回放局数界面
     */
    export class MJGameReplayRoundView extends BaseView {

        /** 单例 */
        private static _onlyOne: MJGameReplayRoundView;
        ...
        public static getInstance():MJGameReplayRoundView {
            if (!this._onlyOne) {
                this._onlyOne = new MJGameReplayRoundView();
            }
        ...
        protected onAddView():void {
            this.roundLabel.text = "第" + MahjongData.requestStartGameMsgAck.currPlayCount + "局";
        }

        private nextRound() {
            let index = MahjongData.requestStartGameMsgAck.currPlayCount + 1;
        ...
        private getPlayerGameLog(handIndex:number) {
            ...
            vGetPlayerGameLogMsg.gameTableID = MahjongLogReplay.gameTableID;
            vGetPlayerGameLogMsg.handIndex = handIndex;
            vGetPlayerGameLogMsg.date = MahjongLogReplay.dateStr.substr(0,10).replace(/-/g,"");
