module FL {
    /**
     * 深圳市天天爱科技有限公司 版权所有
     * @Name:  MahjongBase - MJGameSoundHandler
     * @Description:  //打麻将声音处理
     * @Create: DerekWu on 2017/12/4 11:18
     * @Version: V1.0
     */
    export class MJGameSoundHandler {
        ...
        public static chuCard(cardValue:number, pzOrientation:PZOrientation, withOnTable:boolean = true):void {
            this.playMJGameEffect(pzOrientation, this.getCardEffectRes(cardValue));
            if (withOnTable) {
                this.cardToTable();
            }
        }
        ...
        public static chi(pzOrientation:PZOrientation):void {
            this.playMJGameEffect(pzOrientation, "e_chi_1_mp3");
        }
        ...
        public static peng(pzOrientation:PZOrientation):void {
            this.playMJGameEffect(pzOrientation, "e_peng_1_mp3");
        }
        ...
        public static gang(pzOrientation:PZOrientation):void {
            this.playMJGameEffect(pzOrientation, "e_gang_1_mp3");
        }
        ...
        public static hu(pzOrientation:PZOrientation, isZimo:boolean = false):void {
            if (!isZimo) {
                this.playMJGameEffect(pzOrientation, "e_hu_1_mp3");
            } else {
                //获得性别
                let vSex:number = MJGameHandler.getGamePlayerInfo(pzOrientation).sex;
                if (vSex === 1) {
                    //男性暂时没有自摸音效，后续再加
                    this.playMJGameEffect(pzOrientation, "e_hu_1_mp3");
                } else {
         ...
         * @param {number} string
         */
        private static playMJGameEffect(pzOrientation:PZOrientation, resName:string):void {
            //获得性别
            let vSex:number = MJGameHandler.getGamePlayerInfo(pzOrientation).sex;
            if (vSex === 1) {
                //男
