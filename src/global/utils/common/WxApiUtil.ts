module FL {
    ...
    export class WxApiUtil {
         ...
         * HTTPget，强制电脑端
         */
        public static getForcePC(url: string, callback: Function, thisObj: any, errback?: Function) {
            var request: egret.HttpRequest = new egret.HttpRequest();
            request.setRequestHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:43.0) Gecko/20100101 Firefox/43")
            request.open(url, egret.HttpMethod.GET);
            request.once(egret.Event.COMPLETE, (e) => {
                var request = <egret.HttpRequest>e.currentTarget;
                console.log("requet.response:" + request.response);
            ...
            if (errback) {
                request.once(egret.IOErrorEvent.IO_ERROR, errback, thisObj);
            }
            else {
                request.once(egret.IOErrorEvent.IO_ERROR, (e) => {
                    console.log("error : event=" + e);
        ...
        private static getUserInfo(tokenInfo: any) {
            ...
            this.get(url, (userinfo)=>{
                if (userinfo.errcode) {
                    ...
                    // 闲聊登陆
                    if (GConf.Conf.useWXAuth == 3) {
                        let vLoginMsg:LoginMsg = CommonHandler.getLastDevLoginMsg();
                        vLoginMsg.qqOpenID = userinfo.unionid;
                        vLoginMsg.wxOpenID = userinfo.openid;
                        console.log("XLIMGURL::", vLoginMsg.password);
        ...
        public static loginWXQuick(xlFlag: boolean = false) {
            if (!xlFlag) {
                (<any>FL.GConf).Conf.useWXAuth = 2;
            }
            if (NativeBridge.intoWXLogin && !xlFlag) {
                return;
            }
